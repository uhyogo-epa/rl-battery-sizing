...
class RenewableEnergyEnv:
    ...
    #     solar_radiation_tensor = torch.tensor(solar_radiation, dtype=torch.float32)
    #     bidding_tensor = torch.tensor(bidding, dtype=torch.float32)
     
    #     # 最小二乗誤差を計算し、負の値を報酬として返す
    #     mse_loss = F.mse_loss(solar_radiation_tensor, bidding_tensor)
    #     reward = -mse_loss.item()  # .item() でスカラー値に変
    #     return reward
