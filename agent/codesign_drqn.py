    ...
    def train(self, env, 
        ...
        for episode in iterator:
            ...
            while not done:
                ...
                # Get action
                a, h, c = self.critic.sample_action(torch.from_numpy(obs).float().to(device).unsqueeze(0).unsqueeze(0), torch.tensor(rho).unsqueeze(0).unsqueeze(0).unsqueeze(0),
                ...
                with torch.no_grad():
                    episode_q0, _, _ = self.critic(torch.from_numpy(obs).float().to(device).unsqueeze(0).unsqueeze(0), torch.tensor(rho).unsqueeze(0).unsqueeze(0).unsqueeze(0),h, c)
